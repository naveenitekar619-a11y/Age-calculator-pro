...
export const AgeCalculatorForm: React.FC<AgeCalculatorFormProps> = ({
  ...
  return (
    <div
      id={id}
      className={`bg-white rounded-3xl border border-slate-200/80 shadow-xl p-6 sm:p-8 max-w-4xl mx-auto ${className}`}
    >
      ...
      <form onSubmit={handleCalculate} className="space-y-6">
          ...
          <div className="space-y-2">
            ...
            <div className="relative">
              <input
                ...
                className="w-full px-4 py-3 rounded-xl border border-slate-300 bg-slate-50 text-slate-900 font-medium focus:bg-white focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-all text-base"
                required
              />
            </div>
        ...
        <div className="flex flex-col sm:flex-row items-center gap-3 pt-2">
          <button
            type="submit"
            className="w-full sm:w-auto flex-1 bg-blue-600 hover:bg-blue-700 text-white font-semibold py-3.5 px-6 rounded-xl shadow-md shadow-blue-500/20 hover:shadow-lg transition-all flex items-center justify-center gap-2 text-base active:scale-[0.99]"
          >
          ...
          <button
            type="button"
            onClick={handleReset}
            className="w-full sm:w-auto px-6 py-3.5 rounded-xl border border-slate-300 text-slate-700 font-semibold hover:bg-slate-100 transition-colors flex items-center justify-center gap-2 text-base"
          >
      ...
      {/* RESULTS SECTION */}
      {result && (
        <div className="mt-8 pt-8 border-t border-slate-200 space-y-8 animate-in fade-in slide-in-from-bottom-2 duration-300">
          {/* Main Exact Age Card */}
          <div className="bg-gradient-to-br from-blue-600 to-indigo-700 text-white p-6 sm:p-8 rounded-2xl shadow-lg relative overflow-hidden">
          ...
          {/* Next Birthday Card */}
          <div className="bg-gradient-to-br from-amber-50 to-orange-50 border border-amber-200/80 p-6 rounded-2xl shadow-xs">
            ...
            {result.nextBirthday.isToday ? (
              <div className="bg-amber-500 text-white p-4 rounded-xl text-center shadow-md animate-bounce">
                <span className="text-2xl font-black block">
                  Happy Birthday! 🎉🎂
                </span>
                <span className="text-sm text-amber-100 mt-1 block">
            ...
            {result.nextBirthday.isFeb29Note && (
              <p className="text-xs text-amber-800 mt-3 font-medium bg-amber-100/60 p-2.5 rounded-lg border border-amber-200">
                {result.nextBirthday.isFeb29Note}
              </p>
            )}
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div className="bg-slate-50 border border-slate-200 p-6 rounded-2xl shadow-xs">
              ...
              <div className="space-y-3">
                <div className="flex justify-between items-center py-2 border-b border-slate-200 text-sm">
                  <span className="text-slate-600 font-medium">Total Years:</span>
                  <span className="font-bold text-slate-900">
                    {result.totalYears} Years
                  </span>
                </div>
                <div className="flex justify-between items-center py-2 border-b border-slate-200 text-sm">
                  <span className="text-slate-600 font-medium">Total Months:</span>
                  <span className="font-bold text-slate-900">
                    {result.totalMonths.toLocaleString()} Months
                  </span>
                </div>
                <div className="flex justify-between items-center py-2 border-b border-slate-200 text-sm">
                  <span className="text-slate-600 font-medium">Total Weeks:</span>
                  <span className="font-bold text-slate-900">
            ...
            {/* Time Lived Card */}
            <div className="bg-slate-50 border border-slate-200 p-6 rounded-2xl shadow-xs flex flex-col justify-between">
              <div>
                <div className="flex items-center gap-2 mb-4 text-slate-900 font-bold text-lg">
                ...
                <div className="space-y-3">
                  <div className="flex justify-between items-center py-2 border-b border-slate-200 text-sm">
                    <span className="text-slate-600 font-medium">Approx. Hours:</span>
                    <span className="font-bold text-slate-900">
                      {result.approxHours.toLocaleString()} Hours
                    </span>
                  </div>
                  <div className="flex justify-between items-center py-2 border-b border-slate-200 text-sm">
                    <span className="text-slate-600 font-medium">Approx. Minutes:</span>
                    <span className="font-bold text-slate-900">
                      {result.approxMinutes.toLocaleString()} Minutes
                    </span>
                  </div>
                  <div className="flex justify-between items-center py-2 text-sm">
                    <span className="text-slate-600 font-medium">Approx. Seconds:</span>
